...
def load_cube_lut(filepath):
    ...
    Handles non-standard DOMAIN_MIN and DOMAIN_MAX for Log/HDR LUTs.
    """
    ...
    # Domaines par défaut selon la spécification Adobe
    ...
    for line in lines:
        ...
        elif parts[0][0].isdigit() or parts[0][0] == '.' or parts[0][0] == '-':
            if len(parts) == 3:
    ...
    # Normalisation de l'entrée 8-bit (0-255) vers la plage [0.0, 1.0]
    pixel_norm = np.arange(256, dtype=np.float32) / 255.0
    
    # Projection de l'entrée dans le domaine spécifique de la LUT
    # On utilise le canal rouge [0] pour l'indexation globale (standard dans 99.9% des LUTs)
    d_min, d_max = domain_min[0], domain_max[0]
    pixel_mapped = (pixel_norm - d_min) / (d_max - d_min)
    
    # Conversion en coordonnées dans la grille 3D et protection contre les débordements
    idx = np.clip(pixel_mapped * (size - 1), 0, size - 1)

    i0 = np.floor(idx).astype(np.int32)
    i1 = np.clip(i0 + 1, 0, size - 1)
    f  = (idx - i0)

    r0, g0, b0 = np.meshgrid(i0, i0, i0, indexing='ij')
    r1, g1, b1 = np.meshgrid(i1, i1, i1, indexing='ij')
    fr, fg, fb = np.meshgrid(f,  f,  f,  indexing='ij')
    ...
    # Ta ligne d'origine est conservée car elle est parfaite pour garantir une sortie uint8 propre
